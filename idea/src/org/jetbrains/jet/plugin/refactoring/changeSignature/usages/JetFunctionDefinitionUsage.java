...
public class JetFunctionDefinitionUsage extends JetUsageInfo<PsiElement> {
    ...
    public boolean processUsage(JetChangeInfo changeInfo, PsiElement element) {
        ...
        if (changeInfo.isParameterSetOrOrderChanged()) {
            ...
            JetParameterList newParameterList;
            if (isLambda && parametersCount == 0 && ((JetFunctionLiteral) element).getTypeReference() == null) {
                newParameterList = null;
            }
            else if (isLambda && parametersCount == 1) {
                newParameterList = psiFactory.createFunctionLiteralParameterList(
                        changeInfo.getNewParametersSignature(functionDescriptor, isInherited, hasExpectedType, 0)
                );
            }
            else {
                newParameterList = psiFactory.createParameterList(
                        changeInfo.getNewParametersSignature(functionDescriptor, isInherited, hasExpectedType, 0)
                );
            }
